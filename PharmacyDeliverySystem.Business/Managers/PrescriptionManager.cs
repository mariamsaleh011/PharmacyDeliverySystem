...
namespace PharmacyDeliverySystem.Business
{
	public class PrescriptionManager : IPrescriptionManager
	{
		...
		public PrescriptionManager(PharmacyDeliveryContext ctx) => _ctx = ctx;
		...
		public IEnumerable<Prescription> GetByCustomer(int customerId)
			=> _ctx.Prescriptions.Where(p => p.CustomerID == customerId)
				   .Include(p => p.Order)
				   .AsNoTracking().ToList();

		public IEnumerable<Prescription> GetByPharmacy(int pharmacyId)
			=> _ctx.Prescriptions.Where(p => p.PharmId == pharmacyId)
				   .Include(p => p.Order)
				   .AsNoTracking().ToList();

		public IEnumerable<Prescription> GetByOrder(int orderId)
			=> _ctx.Prescriptions.Where(p => p.OrderID == orderId)
				   .AsNoTracking().ToList();
		...
		{
			var e = _ctx.Prescriptions.Find(id);
			if (e == null) return;
			_ctx.Prescriptions.Remove(e);
			_ctx.SaveChanges();
		}
	}
}
